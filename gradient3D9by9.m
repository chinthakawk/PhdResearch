...
fname = strcat(fol,name(1:numel(name)-4),'_3dGradient_',...
    num2str(ms),'x',num2str(ms),'_log.txt');
fid = fopen(fname, 'w');
...
mx = repmat(sob9x9,1,1,ms);     % x sobel
my = permute(mx, [2 1 3]);
mz = permute(mx, [1 3 2]);

% format width for fprintf
% w = max(numel(num2str(max(mx(:)))),numel(num2str(min(mx(:))))); 

fprintf(fid,strcat('\n\nsobel mask x(:,:,1:',num2str(ms),')\n\t'));
for i = 1:ms
   fprintf(fid, '%5d', mx(i,:,1));
   fprintf(fid, '\n\t');
end

fprintf(fid,strcat('\nsobel mask y(:,:,1:',num2str(ms),')\n\t'));
for i = 1:ms
   fprintf(fid, '%5d', my(i,:,1));
   fprintf(fid, '\n\t');
end
