...
fol = '/Users/mac/Documents/KinectData/102/Sica/K1f500/12_findHoles2_neid=.01.010/';
name = '1_1841001_rgbd_Oo_refined_nR_oP=0.1_pDs=5_pD=4.6084_sr_dis=0.001_np=8_nR_oP=0.15_pDs=12_pD=1.2525_refined_newd=0.002.mat';

fid = fopen(strcat(fol,name(1:numel(name)-4),'_log_',...
...
g1 = zeros(size(pc,1),1); % largest angle
gbeg = zeros(size(pc,1),1); % largest angle beginning point
gend = zeros(size(pc,1),1); % largest angle ending point
...
%         fprintf('\ngbeg = %d',gbeg(i));
        ...
        gend(i) = angstart(gendi);
%         fprintf('\ngend = %d',gend(i));
    %     fprintf('\t%f',ang);
    %     fprintf('\t%f\t%d',g(i),gii);

        angle(i) = min((g1(i)-360/numel(r))/(180-360/numel(r)),1);
    end
...
for i = 1:numel(names)
    save(strcat(fol,name(1:numel(name)-4),'_findHoles-neig#=',...
        num2str(n),names{i},'.mat'), names{i});
end
