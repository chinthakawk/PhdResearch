...
% hSub5 = subplot(3,2,[1 3 5]);
% hSurf2 = trisurf(tri,pc(:,1),pc(:,2),pc(:,3),'FaceVertexCData',cdata,'FaceColor','interp','edgeColor','none');
% trisurf(tri,pc(:,1),pc(:,2),pc(:,3),'FaceVertexCData',cdata,'FaceColor','interp','edgeColor','none');
trisurf(tri,pc(:,1),pc(:,2),pc(:,3),'FaceVertexCData',cdata,'edgeColor','none');
...
axis equal; %grid on;
% xlabel('x'); ylabel('y'); zlabel('z');
% title('duck from 4 point clouds with colors');
% view([-170 -70]);
axis off;
