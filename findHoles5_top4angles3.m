...
fol = '/Users/mac/Documents/KinectData/102/Den/K1f301/27_findh_differentpara_20160521/';
name = '1_1437348_rgbd_oo_refined-nR_oP=0.05_pDs=4_pD=3.8519-sr_dis=0.001_np=8-findHoles_neig#=21-fsh_newd=0.001-fshi_acc=3_cube=3.mat';
...
toc;
fprintf(fid,'\n\nElapsed time after neighborhood collection = %f sec',toc);
...
toc;
fprintf(fid,...
    '\n\nElapsed time after neighborhood collection updating = %f sec',...
    toc);
...
%             neid(i,ri) = [];
        ...
        rnan = r(isnan(pc(r,1))==1 | isnan(pc(r,2))==1 |...
            isnan(pc(r,3))==1);
        r(rnan) = [];
...
toc;
fprintf(fid,'\n\nElapsed time after angle criterian = %f sec',toc);
...
toc;
fprintf(fid,'\n\nElapsed time after half disk criterian = %f sec',toc);
...
%     fprintf('boundary = %f / (%f + %f + %f) = %f\n',piboundary(i),...
%         piinterior(i), picorner(i), piline(i), boundary(i));
%     fprintf('boundary = %f\n',boundary(i));
...
toc;
fprintf(fid,'\n\nElapsed time after boudary criterian = %f sec',toc);
...
toc;
fprintf(fid,'\n\nElapsed time after combining criterian = %f sec',toc);
...
for i = 1:numel(names)
    save(strcat(fol,name(1:numel(name)-4),'-findsh_neig#=',...
        num2str(n-1),'_neirad=',num2str(neirad),'_',names{i},'.mat'),...
        names{i});
end
