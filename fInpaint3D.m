...
% 
% 20160413: can we extend 2d inpainting to 3d

function [yq,xq,in] = fInpaint3D()

tic;

clc; clear all;

fol = '/Users/mac/Documents/NCU/Research/Progress Presentations/20160321_RGBDvsPC/02_RgbF1/K1/12_3dInpaint/';
...
% figure;
% hax1 = axes;
% scatter3(hax1,pc(:,1),pc(:,2),pc(:,3),ss,pc(:,4:6)/255);
% xlabel('x');
% ylabel('y');
% zlabel('z');
% view(2);
...
function [yq,xq,in] = findHoles(pc,acc)

pc(:,1:3) = round(pc(:,1:3)*10^acc,0);
xmin = min(pc(:,1));
xmax = max(pc(:,1));
ymin = min(pc(:,2));
ymax = max(pc(:,1));
...
fprintf('\n%d\t%d\t%d\t%d',xmin,xmax,ymin,ymax,h,w);
...
se = strel('disk',2);
% holes = imerode(holes,se);
holes1 = imdilate(holes,se);
se = strel('disk',1);
holes2 = imerode(holes1,se);
se = strel('disk',6);
holes3 = imerode(holes2,se);

% imshow(imcomplement(holes1));
...
% xq(xq==0) = [];
% yq(yq==0) = [];
in = inpolygon(xq,yq,c(k),r(k));


holes4 = zeros(h,w);
holes4(yq(in),xq(in)) = 1;
imshow(holes1);
hold on;
plot(c(k),r(k));
